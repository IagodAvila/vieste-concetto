...
function EmptyPanel({ close, text, title }: { close: () => void; text: string; title: string }) { return <div className="flex flex-1 flex-col items-center justify-center gap-5 px-3 text-center"><p className="font-serif text-5xl">{title}</p><p className="text-sm text-muted-foreground">{text}</p><a onClick={close} className="eyebrow bg-clay px-8 py-4 text-white transition-colors hover:bg-graphite" href="#novidades">Ver novidades</a></div>; }
