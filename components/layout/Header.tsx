...
const collections = [
  { label: "Movimento 01", id: "movimento-01" },
  { label: "Movimento 02", id: "movimento-02" },
  { label: "Essenciais", id: "essenciais" },
];
    ...
    <>
          ...
          <nav className="hidden items-center gap-7 lg:flex" aria-label="Navegação principal">
            ...
            <div className="group relative">
              ...
              <div className="pointer-events-none absolute top-full left-0 w-56 translate-y-1 border border-border bg-background pt-4 pb-3 opacity-0 shadow-sm transition duration-300 group-hover:pointer-events-auto group-hover:translate-y-0 group-hover:opacity-100">
                {collections.map((item) => <a className="block px-5 py-2 text-sm transition-colors hover:bg-secondary" href={`/#${item.id}`} key={item.id}>{item.label}</a>)}
              </div>
            </div>
          </nav>
      ...
      <aside className={`fixed inset-0 z-60 flex flex-col bg-peach-soft transition-[opacity,transform] duration-500 lg:hidden ${overlay === "menu" ? "translate-x-0 opacity-100" : "pointer-events-none -translate-x-4 opacity-0"}`} aria-hidden={overlay !== "menu"} aria-label="Menu">
        ...
        <nav className="flex-1 overflow-y-auto px-6 pt-6">
          {navItems.map((item) => <a onClick={closeOverlay} className="block py-3 font-serif text-6xl" href={`/#${item.toLowerCase().replaceAll(" ", "-")}`} key={item}>{item}</a>)}
          <Link onClick={closeOverlay} className="block py-3 font-serif text-6xl" href="/contato">Contato</Link>
          <p className="eyebrow mt-10 text-clay">Coleções</p>
          <div className="mt-4 space-y-1 border-t border-clay/20 pt-4">{collections.map((item) => <a onClick={closeOverlay} className="block py-1.5 text-sm" href={`/#${item.id}`} key={item.id}>{item.label}</a>)}</div>
...
function EmptyPanel({ close, text, title }: { close: () => void; text: string; title: string }) { return <div className="flex flex-1 flex-col items-center justify-center gap-5 px-3 text-center"><p className="font-serif text-5xl">{title}</p><p className="text-sm text-muted-foreground">{text}</p><Link onClick={close} className="button-primary eyebrow px-8 py-4" href="/novidades">Ver novidades</Link></div>; }
