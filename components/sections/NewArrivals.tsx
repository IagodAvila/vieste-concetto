...
function ProductCard({ product, onOpen }: { product: Product; onOpen: () => void }) {
  ...
  return (
    <article className="group relative">
      ...
      <div className="mt-4 space-y-1"><h3 className="text-[.95rem] leading-snug font-medium"><button className="link-underline cursor-pointer text-left" onClick={onOpen} type="button">{product.name}</button></h3><p className="text-xs tracking-wide text-muted-foreground">{product.category} · {product.color}</p><p className="pt-1 text-sm">{product.price}</p><p className="text-xs text-muted-foreground">{product.installments}</p></div>
    ...
    <div className="fixed inset-0 z-70 overflow-y-auto bg-graphite/55 p-0 backdrop-blur-sm md:p-6" role="dialog" aria-modal="true" aria-labelledby={`product-title-${product.slug}`} onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <div className="relative mx-auto grid min-h-dvh max-w-6xl bg-background shadow-2xl md:min-h-0 md:grid-cols-2">
        ...
        <div className="bg-secondary p-3 md:p-6">
          <div
            className="relative aspect-square min-w-0 overflow-hidden bg-background md:cursor-zoom-in"
            onMouseEnter={() => setZoomed(true)}
            onMouseLeave={() => setZoomed(false)}
            onMouseMove={(event) => {
              const bounds = event.currentTarget.getBoundingClientRect();
              setZoomPosition({
                x: Math.min(80, Math.max(20, ((event.clientX - bounds.left) / bounds.width) * 100)),
                y: Math.min(80, Math.max(20, ((event.clientY - bounds.top) / bounds.height) * 100)),
              });
            }}
          >
            <Image
        ...
        <div className="relative flex flex-col p-6 md:max-h-[calc(100vh-3rem)] md:overflow-y-auto md:p-10">
          ...
          <h2 id={`product-title-${product.slug}`} className="mt-3 pr-10 font-serif text-3xl">{product.name}</h2>
          ...
          <button onClick={() => addToCart(product.slug)} className="eyebrow mt-8 w-full bg-clay px-8 py-4 text-white transition-colors hover:bg-graphite" type="button">{quantity ? `Adicionar mais uma · ${quantity} na sacola` : "Adicionar à sacola"}</button>
        </div>
      </div>
    </div>
  );
